...
- Added help-mode status bar hints: `j/k: scroll | g/G: top/bottom | q/esc: close`
- Files: update.go, view.go

### 2026-03-20 - Free navigation, smart backtick, configurable skips, search UX
- **Removed home directory lock**: Navigation is now unrestricted by default. Use `root_path` in config to set a boundary if wanted.
- **Smart backtick**: `` ` `` goes to `/mnt/c` on WSL, `/` everywhere else (was: error on non-WSL)

### 2026-03-20 - Search UX overhaul: configurable skips, preview toggle, name-only
- **All skip directories now in config**: Removed all hardcoded skip lists from search.go. Everything lives in `skip_directories` in the config file — users can see and edit the full list (press `,`). New `shouldSkipDir()` helper handles exact names, wildcards (`Python*`, `*Font Driver*`), and absolute paths (`/usr/bin`).
- **ctrl+p**: Toggle preview panel while searching (works locked or unlocked)
- **ctrl+n**: Toggle name-only vs full-path matching in recursive/ultra search
- Search header shows `(name)` indicator when name-only is active
- All added to help screen, search hints (width-responsive), and README keybindings
- Files: update.go, view.go, model.go, internal/search/search.go, internal/search/search_test.go, internal/config/config.go, README.md

### 2026-03-20 - Fixed editor opening over SSH (vim/nvim)
- Terminal editors (vim, nvim, nano, etc.) now use `tea.ExecProcess` to properly suspend the TUI and hand over the terminal, instead of `cmd.Start()` which launched them backgrounded with no stdio
- GUI editors (code, cursor, zed) still use `cmd.Start()` as before
- Added `isTerminalEditor()` helper and consolidated editor logic into `execEditor()`
- Removed dead code: `editFile`, `editFileAtLine`, `openFile`
- Added scout shell wrapper to droplet `~/.zshrc` for ctrl+g support
- Deployed updated binary to droplet (`/usr/local/bin/scout`)
- Files: helpers.go

### 2026-03-19 - Fixed Enter key in search mode
- Enter on a file in locked search results now navigates to the file's parent directory (same as `f` key) instead of opening it in an editor
- Updated all hints: search header, status bar (search-aware), help screen now documents `enter` in search section
- Files: update.go, view.go

### 2026-03-17 - README audit and update
- Added missing keybindings: `f` (navigate), `` ` `` (WSL /mnt/c), `w/s` (preview scroll), `r` (refresh)
- Fixed Tab cycle to show all 4 search modes (was missing Ultra)
- Added mouse support to "What it does" section
- Fixed config table: `root_path` default was wrong (`$HOME` → `""`), added `show_hidden` and `preview_enabled`
- Added multi-file clipboard mention to file operations
- Files: README.md

### 2026-03-17 - Added AGPL-3.0 License
- Added LICENSE file (GNU Affero General Public License v3.0)
